...
class Product{
    ...
    public function updateProduct($id_SP,$TenSP,$HinhAnh,$LoaiSP,$HangSP,$Gia,$GiaGoc,$AnHien,$MoTa){
        $this->db->query("UPDATE sanpham set TenSP='$TenSP',HinhAnh='$HinhAnh',LoaiSP='$LoaiSP',HangSP='$HangSP',Gia='$Gia',GiaGoc='$GiaGoc',AnHien='$AnHien',MoTa='$MoTa' WHERE id_SP = $id_SP");
        $this->db->execute();
    }
    ...
    public function productsBrandType(){
        ...
        INNER JOIN hang on sanpham.HangSP=hang.id_Hang WHERE sanpham.AnHien = 1");
        return $this->db->pdo_query();
    }
